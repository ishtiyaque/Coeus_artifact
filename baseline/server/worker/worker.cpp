...
#define DB_ELEMENTS ((N * SCALE_FACTOR) * (RESPONSE_CT))

#define THREAD_PER_QUERY ((NUM_THREAD / SCALE_FACTOR))
...
clock_t total_cpu_start_time, total_cpu_end_time;
...
int main(int argc, char **argv)
{
    ...
    stringstream dummy_stream(serialized_query[0]);
    ...
    {
        stringstream qss(serialized_query[i]);
        query[i].load(context, qss);
    }
    ...
    for (int i = 0; i < NUM_THREAD; i++)
    {
        ...
        args[i].query = query[i / (NUM_THREAD / SCALE_FACTOR)];
    }
...
void sendQuery(string _serialized_gal_key, vector<string> _serialized_query)
{
    ...
    serialized_query = _serialized_query;
    //cout<<"received query; size "<<serialized_query.size()<<endl;
    pthread_mutex_lock(&request_received_lock);
...
void *thread_multiply(void *arg)
{
    ...
    int rot_per_thread = floor((double)N / (THREAD_PER_QUERY));
    int query_idx = my_id / THREAD_PER_QUERY;
    int start_rot = ((my_id % THREAD_PER_QUERY) >> 1) * rot_per_thread;
    int remaining = (N / 2) % (THREAD_PER_QUERY / 2);
    if (remaining > ((my_id % THREAD_PER_QUERY) >> 1))
    {
        start_rot += ((my_id % THREAD_PER_QUERY) >> 1);
        rot_per_thread++;
    }
    else
    {
        ...
        for (int i = start_rot; i < end_rot; i++)
        {
...

            {
                ...
                while(tot_rot != i) {
                    ...
                    //printf("next rot %d\n", next_rot);
                    evaluator->rotate_rows_inplace(temp_q_ct, next_rot, gal_keys);
                    tot_rot += next_rot;
                }
            }
            ...
            {
                //time_start = chrono::high_resolution_clock::now();
                evaluator->multiply_plain(temp_q_ct, encoded_db[(query_idx * RESPONSE_CT * N) + res_idx * N + (iter * N / 2) + (i - start_rot)], t_arg->thread_result[res_idx]);
                //time_end = chrono::high_resolution_clock::now();
            ...
            {
                //time_start = chrono::high_resolution_clock::now();
                evaluator->multiply_plain(temp_q_ct, encoded_db[(query_idx * RESPONSE_CT * N) + res_idx * N + (iter * N / 2) + (i - start_rot)], temp_ct);
                //time_end = chrono::high_resolution_clock::now();
                //mult_time += chrono::duration_cast<chrono::microseconds>(time_end - time_start).count();
...
void printReport()
{
    ...
    cout << "Total CPU time (sec) " << endl
         << ((float)total_cpu_end_time - total_cpu_start_time) / CLOCKS_PER_SEC;
